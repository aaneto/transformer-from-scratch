...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float):
        ...
        # Apply sine to even positions and cossine to uneven positions (paper formula)
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        # (Batch, Seq_len, d_model) --> (Batch, Seq_len, d_model) for all
        query = self.w_q(q)
        key = self.w_k(k)
        value = self.w_v(v)

        # Split d_model(512) into smaller heads to combine into the main head
...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask):
        for layer in self.layers:
            x = layer(x, mask) # Layer returns None
        return self.norm(x)
